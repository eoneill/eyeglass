...
function resolveAsset(eyeglass, sass, sassUtils, registeredAssetsMap, relativePathString, cb) {
  ...
  if (data) {
    ...
    var fullUri = httpJoin(
      ...
      eyeglass.options.eyeglass.assets.relativeTo ? "/" : eyeglass.options.eyeglass.httpRoot,
...
module.exports = function(eyeglass, sass) {
  ...
  mainCollection.install = function(assetFile, assetUri, cb) {
    var httpRoot = eyeglass.options.eyeglass.httpRoot;
    if (eyeglass.options.eyeglass.buildDir) {
      if (assetUri.substring(0, httpRoot.length) === httpRoot) {
        assetUri = assetUri.substring(eyeglass.options.eyeglass.httpRoot.length);
      }
      ...
      var dest = path.join(eyeglass.options.eyeglass.buildDir, assetUri);
