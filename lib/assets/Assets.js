...
Assets.prototype.resolveAsset = function($assetsMap, $uri, cb) {
  ...
  var data = resolveAssetDefaults.call(this, $assetsMap, uri.path());

  if (data) {
    var filepath = data.coerce.get("filepath");

    // create the URI
    var fullUri = uriUtils.simple(
    ...
    // normalize the uri
    fullUri = uriUtils.normalize(fullUri);
...
Assets.prototype.install = function(file, uri, cb) {
  ...
  if (options.buildDir) {
    ...
    // normalize the uri using the OS path separator
    uri = uriUtils.normalize(uri, path.sep);
