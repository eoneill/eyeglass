...
var EyeglassModule = require("./eyeglass_module");
var debug = require("./debug");
var path = require("path");
var merge = require("lodash.merge");
var semver = require("semver");
var archy = require("archy");
var uriUtils = require("./uri");
...
EyeglassModules.prototype.findByPath = function(dir) {
  ...
  var cache = this.cache.foundByPath;
  // check for cached result
  if (cache[dir] !== undefined) {
    return cache[dir];
  }

  // in our list of modules, find the module with a matching path
  this.list.some(function(mod) {
    if (mod.path === dir) {
      // ensure we always return the final module reference
      found = this.find(mod.name);
      return;
    }
  }.bind(this));
...
function discoverModules(options) {
  ...
  if (pkg.data) {
    // always include the `dependencies`
    merge(dependencies, pkg.data.dependencies);
    // only if it's the root of the project...
    if (options.isRoot) {
      // include the `devDependencies` and `peerDependencies`
      merge(dependencies, pkg.data.devDependencies, pkg.data.peerDependencies);
    }
  }
  ...
  dependencies = Object.keys(dependencies).reduce(function(modules, dependency) {
    ...
    var resolvedPkg = resolveModulePackage(
      ...
      pkg.path,
      uriUtils.normalize.system(options.dir)
    );
    if (!resolvedPkg) {
